...
function TrocaReinoModal({ isOpen, onClose, onReinoEscolhido, user }) {
  ...
  const handleEscolha = async (reino) => {
    if (!user?.id) {
      alert("Usuário inválido ou não autenticado.");
      return;
    }

    const { permitido, mensagem } = await podeTrocarReino(user.id);

    if (!permitido) {
      alert(mensagem || "Ainda não é possível trocar de reino.");
      return;
    }

    const { success } = await registrarTrocaDeReino(user.id, reino.id, reino.nome);
  ...
  return (
    <div className="escolha-reino-overlay" onClick={onClose}>
      <div className="escolha-reino-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <h2>TROCANDO DE REINO</h2>
        <p>Aponte o destino do seu novo lar</p>

        <div className="reinos-container-esteira">
          {reinos.map((reino) => (
            <div key={reino.id} className="reino-card">
              <div className="reino-card-background" style={{ backgroundImage: `url(${reino.cardImage})` }}></div>
              ...
              <div className="reino-card-content">
                ...
                <button className="reino-escolher-btn" onClick={() => handleEscolha(reino)}>
                  <img src={reino.buttonImage} alt={`Jurar Lealdade a ${reino.nome}`} loading="lazy" />
                </button>
              </div>
            </div>
          ))}
...
export default TrocaReinoModal;
